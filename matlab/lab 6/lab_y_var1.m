...
%% ====================== ЧАСТИНА 1: z1(x,y) ======================
fprintf('==========================================================\n');
fprintf('   ЧАСТИНА 1: z1(x,y) = sin(2*sqrt(x²+y²)) / (sqrt(x²+y²) + 0.001)\n');
fprintf('==========================================================\n');

N = 500;
% Генерація точок по діагоналі квадрата [0, pi/2] x [0, pi/2]
x1 = linspace(0, pi/2, N);
y1 = linspace(0, pi/2, N);
Input1  = [x1; y1];
Output1 = sin(2*sqrt(x1.^2 + y1.^2)) ./ (sqrt(x1.^2 + y1.^2) + 0.001);
...
print_table(configs, results1, 'z1(x,y) = sin(2*sqrt(x²+y²))/(sqrt(x²+y²)+0.001)');
...
figure('Name','Порівняння похибок — Варіант 1','Position',[100 100 900 500]);
...
title('Варіант 1 — Порівняння точності мереж');
legend({'z1(x,y) = sin(2√(x²+y²))/(√(x²+y²)+0.001)', 'y(x) = sin(x)+cos(x/2)'}, ...
       'Location','northwest');
grid on;

% 2. Детальні графіки апроксимації для z1
figure('Name','Детальна апроксимація — z1','Position',[100 100 1200 700]);
sgtitle('Варіант 1 — z1(x,y) = sin(2√(x²+y²))/(√(x²+y²)+0.001)', 'FontSize',14);
for i = 1:6
    subplot(3,2,i);
...
% 3. Детальні графіки апроксимації для y(x)
figure('Name','Детальна апроксимація — y(x)','Position',[100 100 1200 700]);
sgtitle('Варіант 1 — y(x) = sin(x)+cos(x/2)', 'FontSize',14);
for i = 1:6
    subplot(3,2,i);
...
% 4. 3D поверхня z1 (краща мережа)
[~, best1] = min(results1);
Ng = 40;
[xg,yg] = meshgrid(linspace(0,pi/2,Ng));
Zg_true = sin(2*sqrt(xg.^2+yg.^2)) ./ (sqrt(xg.^2+yg.^2) + 0.001);
Zg_nn   = reshape(nets1{best1}([xg(:)'; yg(:)']), Ng, Ng);

figure('Name','3D Поверхня z1','Position',[100 100 1100 500]);
subplot(1,2,1);
surf(xg,yg,Zg_true,'EdgeColor','none'); title('Еталон'); colorbar; grid on;
xlabel('x'); ylabel('y'); zlabel('z1');
subplot(1,2,2);
surf(xg,yg,Zg_nn,'EdgeColor','none');
title(sprintf('НМ: %s (%.4f%%)', short_labels{best1}, results1(best1))); colorbar; grid on;
xlabel('x'); ylabel('y'); zlabel('z1');
sgtitle('Варіант 1 — 3D поверхня z1(x,y)');
