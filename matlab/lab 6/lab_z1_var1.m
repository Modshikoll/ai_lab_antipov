...
%  Функція: z1(x,y) = sin(2*sqrt(x^2+y^2)) / (sqrt(x^2+y^2) + 0.001)
%  Запуск: >> lab_z1_var1
%% ============================================================
clear; clc; close all;

%% ── Параметри ───────────────────────────────────────────────
f     = @(x,y) sin(2*sqrt(x.^2+y.^2)) ./ (sqrt(x.^2+y.^2) + 0.001);
N_IN  = 6;
N_OUT = 9;
X_MIN = 0; X_MAX = pi/2;
Y_MIN = 0; Y_MAX = pi/2;
...
%% ── Таблиця значень ─────────────────────────────────────────
fprintf('\n=== z1(x,y)=sin(2*sqrt(x²+y²))/(sqrt(x²+y²)+0.001)  [ значення у точках МФ ] ===\n');
fprintf('%10s','');
...
title('Еталонна поверхня  z_1 = sin(2\sqrt{x^2+y^2})/(\sqrt{x^2+y^2}+0.001)','FontSize',11);
...
sgtitle('МФ входу x  —  z_1 = sin(2\sqrt{x^2+y^2})/(\sqrt{x^2+y^2}+0.001)');
...
title('z_1 = sin(2\sqrt{x^2+y^2})/(\sqrt{x^2+y^2}+0.001)  — Порівняння похибок');
grid on;
for k=1:3
...
fprintf('\n╔══════════════════════════════════════════════════════════╗\n');
fprintf('║   ВИСНОВКИ  z1(x,y) = sin(2*sqrt(x²+y²))/(sqrt(x²+y²)+0.001) ║\n');
fprintf('╠══════════════════════════════════════════════════════════╣\n');
