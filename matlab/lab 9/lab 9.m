% ============================================================
% ЛАБОРАТОРНА РОБОТА 4 (9) — Варіант 1
% Моделювання об'єктів управління на основі нейронних мереж
% ============================================================
% Функція 1 (двовимірна): z1(x,y) = sin(2*sqrt(x^2+y^2)) / (sqrt(x^2+y^2) + 0.001)
%   Входи x, y в [0, pi/2]
% Функція 2 (одновимірна): y(x) = sin(x) + cos(x/2)
%   Вхід x в [0, pi]
% ============================================================
...
%% ============================================================
%  ЧАСТИНА 1: z1(x,y) = sin(2*sqrt(x^2+y^2)) / (sqrt(x^2+y^2) + 0.001)
%% ============================================================
fprintf('\n==========================================================\n');
fprintf('   ЧАСТИНА 1:  z1(x,y) = sin(2*sqrt(x^2+y^2)) / (sqrt(x^2+y^2) + 0.001)\n');
fprintf('==========================================================\n');

x1_vec  = linspace(0, pi/2, N);
y1_vec  = linspace(0, pi/2, N);
Input1  = [x1_vec; y1_vec];
r1      = sqrt(x1_vec.^2 + y1_vec.^2);
Output1 = sin(2 * r1) ./ (r1 + 0.001);
...
print_table(configs, results1, 'z1(x,y) = sin(2*sqrt(x^2+y^2))/(sqrt(x^2+y^2)+0.001)');

%% ============================================================
%  ЧАСТИНА 2: y(x) = sin(x) + cos(x/2)
%% ============================================================
fprintf('\n==========================================================\n');
fprintf('   ЧАСТИНА 2:  y(x) = sin(x) + cos(x/2)\n');
fprintf('==========================================================\n');

x2_vec  = linspace(0, pi, N);
Input2  = x2_vec;
Output2 = sin(x2_vec) + cos(x2_vec / 2);
...
print_table(configs, results2, 'y(x) = sin(x)+cos(x/2)');
...
for i = 1:6
    ...
    xlabel('x'); ylabel('y');
    title(sprintf('Апроксимація y(x): %s', short_labels{i}), 'FontSize', 10);
...
title('Порівняння похибок різних конфігурацій нейронних мереж  [y(x) = sin(x)+cos(x/2)]', ...
      'FontSize', 11);
grid on;
...
title('Криві навчання всіх конфігурацій нейронних мереж  [y(x) = sin(x)+cos(x/2)]', ...
      'FontSize', 11);
legend('Location', 'NorthEast', 'FontSize', 9);
grid on;
drawnow;

% --- Графік C: 3D порівняння z1 (еталон vs найкраща НМ) ---
[~, best1] = min(results1);
Ng = 40;
...
Rg = sqrt(Xg.^2 + Yg.^2);
Zg_true = sin(2 * Rg) ./ (Rg + 0.001);
Zg_nn   = reshape(nets1{best1}([Xg(:)'; Yg(:)']), Ng, Ng);

figure('Color', 'w', 'Position', [100 100 1100 460]);
subplot(1,2,1);
surf(Xg, Yg, Zg_true, 'EdgeColor', 'none'); colorbar; grid on;
xlabel('x'); ylabel('y'); zlabel('z1');
title('Еталон: z1(x,y)', 'FontSize', 11);

subplot(1,2,2);
surf(Xg, Yg, Zg_nn, 'EdgeColor', 'none'); colorbar; grid on;
xlabel('x'); ylabel('y'); zlabel('z1');
title(sprintf('НМ: %s  (похибка %.4f%%)', short_labels{best1}, results1(best1)), ...
      'FontSize', 11);

sgtitle('Варіант 1 — 3D поверхня  z1(x,y) = sin(2√(x²+y²)) / (√(x²+y²)+0.001)', ...
